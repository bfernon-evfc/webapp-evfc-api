...
`D:\CLAUDE\CODE\pack-evfc-fpa\skills\<dossier-module>\SKILL.md`.
...
version au modèle Claude — silencieusement.

## Processus

1. Vérifier que le dépôt source existe : `D:\CLAUDE\CODE\pack-evfc-fpa\skills`
...
2. Depuis la racine du projet `webapp-evfc-api` (là où se trouve `index.html`),
   exécuter :
   ```bash
   node .claude/skills/sync-evfc-skill-table/sync-skills.js
   ```
   Le script :
...
- Seul le `SKILL.md` racine de chaque module est embarqué — pas les fichiers
  ...
  fortement de ces fichiers annexes, son comportement réel dans Claude (hors de cette
  webapp) peut différer de ce qui est embarqué ici.
- Le chemin source par défaut (`D:\CLAUDE\CODE\pack-evfc-fpa\skills`) est local à ce
